...
public class EventPublicationInterceptorDemo {
    ...
    public static void main(String[] args) {
        ...
        // 方法一：直接在execute()中发送事件
		staticExecutor.execute();

        // 方法一：基于AOP实现事件发送。
		// EventPublicationInterceptor(定义事件拦截器)
		// Pointcut(指定拦截方法)、
		// PointcutAdvisor(关联事件和拦截方法)
        executor.execute();
    ...
    public static PointcutAdvisor pointcutAdvisor(Pointcut pointcut, EventPublicationInterceptor eventPublicationInterceptor) {
		...
		// pointcut指定拦截Executor.execute()方法。
		// eventPublicationInterceptor指定了拦截方法执行完成后，发送ExecutedEvent事件
