...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ResourcePatternResolver getResourcePatternResolver() {
		// 当前AbstractApplicationContext类 继承 DefaultResourceLoader，因此也是一个资源加载器类。
		// PathMatchingResourcePatternResolver为 Spring资源加载器，其getResource()方法用于加载资源
	...
	public void refresh() throws BeansException, IllegalStateException {
		logger.info("开始refresh应用............");
		synchronized (this.startupShutdownMonitor) {
			// Prepare this context for refreshing.
			//1. 准备Context的刷新(准备上下文的刷新)。调用容器准备刷新的方法，获取容器的当前时间，同时给容器设置同步标识
			prepareRefresh();
			...
			logger.info("执行refresh#prepareBeanFactory............");
			...
			try {
				...
				logger.info("执行refresh#postProcessBeanFactory............");
				postProcessBeanFactory(beanFactory);

				// Invoke factory processors registered as beans in the context.
				//5. 调用所有注册的BeanFactoryPostProcessor的Bean，即调用bean的后置处理器
				...
				// 如：MapperScannerConfigurer，扫描指定包下的Mapper接口，注册成Bean定义。
				// 创建Bean时，对接口生成代理对象，生成FactoryBean对象，放入Bean容器。
				// TODO
				logger.info("执行refresh#invokeBeanFactoryPostProcessors。" +
						"回调BeanFactoryPostProcessor接口的实现类中的postProcessBeanFactory()和postProcessBeanDefinitionRegistry()方法，" +
						"对Bean定义进行操作，比如添加新的BeanDefinition............");
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				//6. 为BeanFactory注册BeanPost事件处理器，BeanPostProcessor是Bean后置处理器，用于监听容器触发的事件
				//调用beanFactory.addBeanPostProcessor(postProcessor)，将后置处理器添加到AbstractBeanFactory.beanPostProcessors集合
				//BeanPostProcessor接口有两个方法，分别在Bean初始化前后执行。在AbstractAutowireCapableBeanFactory.initializeBean()中执行
				logger.info("执行refresh#registerBeanPostProcessors。注册Bean初始化前置和后置处理器。");
				registerBeanPostProcessors(beanFactory);

				// Initialize message source for this context.
				//7. 初始化信息源，和国际化相关
				logger.info("执行refresh#initMessageSource............");
				initMessageSource();
				...
				logger.info("执行refresh#onRefresh............");
				onRefresh();
				...
				//12. 初始化容器的生命周期事件处理器，并发送容器的生命周期事件（Spring Cloud也是从这里启动的）
				logger.info("执行refresh#finishRefresh............");
				finishRefresh();
			}
	...
	/**
	 * 该方法会解析所有 Spring 配置文件（通常我们会放在 resources 目录下），将所有 Spring 配置文件中的 bean 定义封装成 BeanDefinition，
	 * 加载到 BeanFactory 中。常见的，如果解析到<context:component-scan base-package="" /> 注解时，会扫描 base-package 指定的目录，
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 将数据存放到DefaultListableBeanFactory.resolvableDependencies集合中，在DefaultListableBeanFactory.findAutowireCandidates()方法中，可以注入到需要的Bean中
		// 可以通过自动绑定查找
